...
const Group: FC<GroupProps> = ({ user }) => {
        ...
        const unsubscribe: any = getMessageSnaphot({
            setMessages: (newMessages: any) => {
                ...
                const filteredMessages = newMessages.filter(
                    (message: any) => message.channel.label === channel.label && message.groupId === id
                );
                // Sort the filtered messages by timestamp
                const sortedMessages = filteredMessages.slice().sort((a: any, b: any) => {
    ...
    const MessageItem: FC<{ index: number, style: React.CSSProperties }> = ({ index, style }) => {
        const message: any = messages[index];

        return (
            <div style={style} className="self-center pt-8">
                <Message
                    key={message.id}
                ...
                <div className="w-full max-w-screen-xl">
                    <div className="flex justify-center items-center mb-6 top-3">
                        ...
                        <div className="flex justify-center items-center">
                            <h1 className="text-5xl top-3 absolute self-center">{groupData.name}</h1>
                        </div>
                    ...
                    <div className="py-8" /> {/* A space between the button and the menu */}
