...
export const createGroup = async ({ channels, name, members }: { channels: any, name: string, members: any[] }) => {
    try {
        const cleanedMembers = members.map(
            member => member.uid.replace(/[^a-zA-Z0-9]/g, '')
        );
